...
class Profile:
    ...
    @property
    def openself_analysis(self) -> Dict[str, Any]:
        """Get the OpenSelf analysis section."""
        return self.get_field('openself_analysis', {})
    
    @property
    def has_openself_data(self) -> bool:
        """Check if profile has OpenSelf analysis data."""
        return bool(self.openself_analysis and 'personality_analysis' in self.openself_analysis)
    
    @property
    def openself_personality(self) -> Dict[str, Any]:
        """Get OpenSelf personality analysis."""
        return self.get_field('openself_analysis.personality_analysis', {})
    
    @property
    def openself_fingerprint(self) -> Dict[str, Any]:
        """Get OpenSelf vocabulary fingerprint."""
        return self.get_field('openself_analysis.vocabulary_fingerprint', {})
    
    @property
    def openself_soul_profile(self) -> str:
        """Get OpenSelf SOUL.md profile."""
        return self.get_field('openself_analysis.soul_profile', '')
    
    def get_openself_trait(self, trait_name: str, default: Any = None) -> Any:
        """Get a specific OpenSelf personality trait."""
        return self.get_field(f'openself_analysis.personality_analysis.{trait_name}', default)
    
    def get_combined_personality_info(self) -> Dict[str, Any]:
        """
        Get combined personality information from both questionnaire and OpenSelf analysis.
        
        Returns:
            Dictionary with combined personality data
        """
        combined = {
            'questionnaire_traits': self.personality_traits,
            'openself_available': self.has_openself_data,
        }
        
        if self.has_openself_data:
            combined.update({
                'openself_personality': self.openself_personality,
                'openself_fingerprint': self.openself_fingerprint,
                'communication_style': {
                    'emoji_frequency': self.get_openself_trait('emoji_frequency', 0),
                    'formality': self.get_openself_trait('formality', 'Unknown'),
                    'primary_language': self.get_openself_trait('primary_language', 'Unknown'),
                    'uses_slang': self.get_openself_trait('uses_slang', False),
                },
                'writing_style': {
                    'capitalization': self.get_field('openself_analysis.vocabulary_fingerprint.capitalization_style', 'Normal'),
                    'avg_word_length': self.get_field('openself_analysis.vocabulary_fingerprint.avg_word_length', 0),
                    'unique_words_ratio': self.get_field('openself_analysis.vocabulary_fingerprint.unique_words_ratio', 0),
                }
            })
    ...
    def questionnaire(self) -> Dict[str, Any]:
        """Get questionnaire-derived profile data."""
        return self.get_field('questionnaire', {})

    @property
    def strengths(self) -> List[str]:
        """Get strengths from questionnaire or profile-derived data."""
        return self.get_field('questionnaire.strengths', [])

    @property
    def weaknesses(self) -> List[str]:
        """Get weaknesses from questionnaire or profile-derived data."""
        return self.get_field('questionnaire.weaknesses', [])

    @property
    def preferences(self) -> Dict[str, Any]:
        """Get user preferences from questionnaire or profile-derived data."""
        return self.get_field('questionnaire.preferences', {})

    def get_user_profile_summary(self) -> str:
        """Return a short summary of the questionnaire-derived user profile."""
        summary_parts = []
        if self.name:
            summary_parts.append(f"Name: {self.name}")
        if self.username:
            summary_parts.append(f"Username: {self.username}")
        if self.basic_info:
            summary_parts.append(f"Basic Info: {', '.join([f'{k}: {v}' for k,v in self.basic_info.items()][:3])}")
        if self.personality_traits:
            summary_parts.append(f"Personality: {', '.join([f'{k}: {v}' for k,v in self.personality_traits.items()][:3])}")
        if self.strengths:
            summary_parts.append(f"Strengths: {', '.join(self.strengths[:3])}")
        if self.weaknesses:
            summary_parts.append(f"Weaknesses: {', '.join(self.weaknesses[:3])}")
    ...
    def get_custom_settings(self) -> Dict[str, Any]:
        ...
        base_fields = {'required', 'basic_info', 'personality_traits', 'response_styles', 
                      'relationships', 'knowledge_and_expertise', 'sample_conversations', 'off_topic_message', 'questionnaire'}
...
"""
        ...
        preferred_order = [
            ...
            ('openself_analysis', 'OPENSELF ANALYSIS'),
        ]
        ...
        # Add OpenSelf analysis summary if available
        if self.has_openself_data:
            openself_summary = []
            if self.get_openself_trait('formality'):
                openself_summary.append(f"**OpenSelf Formality:** {self.get_openself_trait('formality')}")
            if self.get_openself_trait('primary_language'):
                openself_summary.append(f"**Primary Language:** {self.get_openself_trait('primary_language')}")
            if self.get_openself_trait('uses_slang') is not None:
                openself_summary.append(f"**Uses Slang:** {'Yes' if self.get_openself_trait('uses_slang') else 'No'}")
            if openself_summary:
                summary_parts.extend(openself_summary)
