...
class ProfileManager:
    ...
    def load_profile(self, file_path: Union[str, Path], profile_name: Optional[str] = None) -> Profile:
        ...
        try:
            ...
            if profile_name is None:
                # For nested profile structures, try to get the profile name from the data
                if "profiles" in raw_data and isinstance(raw_data["profiles"], dict):
                    if len(raw_data["profiles"]) == 1:
                        profile_name = list(raw_data["profiles"].keys())[0]
                    elif "default" in raw_data["profiles"]:
    ...
    def ingest_questionnaire_answers(
        self,
        profile_name: str,
        ...
        """Create or update a profile from questionnaire answers."""
        from .profile_intake import ProfileIntakePipeline

        intake_pipeline = ProfileIntakePipeline(self)
        return intake_pipeline.create_profile_from_questionnaire(
            profile_name=profile_name,
            answers=answers,
    ...
    def ingest_questionnaire_and_chat_analysis(
        self,
        profile_name: str,
        answers: Union[Dict[str, Any], List[Tuple[str, Any]]],
        ...
        """
        Create a comprehensive profile from questionnaire answers and chat analysis.

        Args:
            profile_name: Name for the new profile
            answers: Questionnaire answers
            chat_messages: Optional chat messages for personality analysis
            user_info: Optional user information for enhanced analysis
            base_profile_name: Optional base profile to merge with
            file_path: Optional file path to save to

        Returns:
            Comprehensive Profile instance
        """
        from .profile_intake import ProfileIntakePipeline

        intake_pipeline = ProfileIntakePipeline(self)
        return intake_pipeline.create_profile_from_questionnaire_and_chat(
            profile_name=profile_name,
            answers=answers,
    ...
    def enhance_profile_with_chat_analysis(
        ...
        Args:
            profile_name: Name of profile to enhance
        ...
        Returns:
            Enhanced Profile instance or None if profile not found
        """
        profile = self.get_profile(profile_name)
        if not profile:
            return None
        ...
        intake_pipeline = ProfileIntakePipeline(self)
        return intake_pipeline.enhance_profile_with_chat_analysis(
            profile=profile,
            chat_messages=chat_messages,
            user_info=user_info,
            save=save,
