...
class ResponseGenerator:
    ...
    def __init__(
        ...
        self.openself_integration = None
        if OpenSelfIntegration and profile and profile.has_openself_data:
            try:
                self.openself_integration = OpenSelfIntegration()
                self.logger.info("OpenSelf integration enabled for response generation")
            except Exception as e:
                self.logger.warning(f"Failed to initialize OpenSelf integration: {e}")
    ...
    async def generate_response(
        ...
        try:
            ...
            # Step 3.5: Enhance response with OpenSelf integration if available
            if self.openself_integration and self.profile:
                try:
                    generated_response = self.openself_integration.enhance_response_with_profile(
                        generated_response, self.profile
                    )
                    self.logger.debug("Response enhanced with OpenSelf integration")
                except Exception as e:
                    self.logger.warning(f"OpenSelf enhancement failed: {e}")
